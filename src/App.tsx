            ...
            >
              ...
              >
                ...
                <div className="pt-20 pb-10 px-8 text-center">
                  ...
                  {(selectedPerson.instagram || selectedPerson.tiktok) && (
                    <div className="flex flex-wrap justify-center gap-3 mb-6">
                      {selectedPerson.instagram && (
                        <a
                          href={(() => {
                            const clean = selectedPerson.instagram.trim();
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1.5 bg-gradient-to-tr from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-all shadow-md shadow-pink-500/10 active:scale-95 cursor-pointer"
                        >
                          <Instagram size={14} />
                          <span>{selectedPerson.instagram.startsWith('@') ? selectedPerson.instagram : `@${selectedPerson.instagram}`}</span>
                        </a>
                      )}
                      {selectedPerson.tiktok && (
                        <a
                          href={(() => {
                            const clean = selectedPerson.tiktok.trim();
                            if (clean.startsWith('http://') || clean.startsWith('https://')) return clean;
                            const raw = clean.startsWith('@') ? clean.slice(1) : clean;
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-1.5 bg-black hover:bg-zinc-900 text-white font-bold text-xs px-4 py-2.5 rounded-xl transition-all shadow-md shadow-black/10 active:scale-95 border border-zinc-850 cursor-pointer"
                        >
                          <svg className="w-3.5 h-3.5 fill-white" viewBox="0 0 24 24">
                            <path d="M12.53.02C13.84 0 15.14.01 16.44 0c.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.17-2.89-.6-4.09-1.5-.7-.52-1.28-1.19-1.74-1.95-.01 2.25-.01 4.51-.01 6.77-.04 2.1-.51 4.25-1.68 6.03-1.61 2.49-4.59 3.99-7.58 3.66-3.41-.37-6.27-3.08-6.72-6.51-.55-4.14 2.12-8.19 6.21-8.91.82-.14 1.67-.16 2.5-.04V7.54c-1.13-.19-2.33-.03-3.39.46-1.97.91-3.3 2.99-3.26 5.17-.02 2.3 1.48 4.45 3.65 5.2 2.17.75 4.73.08 6.16-1.71 1.01-1.26 1.41-2.92 1.34-4.52V.02h.16z"/>
                          </svg>
                          <span>{selectedPerson.tiktok.startsWith('@') ? selectedPerson.tiktok : `@${selectedPerson.tiktok}`}</span>
                        </a>
                      )}
                    </div>
                  )}
